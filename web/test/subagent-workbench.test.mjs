...
const { render } = await import(`data:text/javascript;base64,${Buffer.from(bundle.outputFiles[0].contents).toString('base64')}`).catch(error => { throw new Error(error.message); });

test('historical paused state never headlines a resumed, completed or unconfirmed task', () => {
  for (const [state, label] of [['running', '运行中'], ['completed', '执行结束'], ['unknown', '状态待确认']]) {
    const html = render(state);
    assert.equal((html.match(new RegExp(`subagent-status state-${state}">${label}`, 'g')) ?? []).length, 2);
...
test('refresh failure invalidates all headline states and unrelated tool IDs do not attach', () => {
  assert.equal((render('running', true).match(/subagent-status state-unknown">状态待确认/g) ?? []).length, 2);
  assert.match(render('running', false, false), /subagent-status state-unknown">状态待确认/);
});

test('card and inspector show completed subtasks, workflow warning and diagnostic together', () => {
  const extra = { steps: [{ effectiveState: 'completed' }, { effectiveState: 'completed' }], childSummary: { total: 2, completed: 2 },
    workflow: { state: 'failed', issue: 'continuation', error: 'unsupported-continuation: continuation was not persisted.' } };
  const html = render('failed', false, true, extra);
  assert.equal((html.match(/subagent-status state-completed">子任务已交还结果/g) ?? []).length, 2);
  assert.equal((html.match(/class="subagent-flow-warning">流程待恢复/g) ?? []).length, 2);
  assert.match(html, /已记录的 2\/2 个子任务均已交还结果/);
  assert.match(html, /unsupported-continuation/);
  assert.doesNotMatch(render('failed', true, true, extra), /subagent-status state-completed/);
});
...
test('summary aligns to composer width while the label and popover stay compact', async () => {
  ...
  assert.match(rule('composer'), /max-width: var\(--composer-max-width, 800px\)/);
  assert.match(rule('subagent-summary'), /max-width: var\(--composer-max-width, 800px\)/);
