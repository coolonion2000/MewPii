...
// Import compiled output: the source re-exports siblings with .js specifiers, which Node's
// type-stripping cannot resolve against .ts files. `npm test` builds dist before running.
import {
  projectState,
  presentRun,
  reportsUnmetCriteria,
} from "../dist/subagent-presentation.js";
...
test("newer transcript invalidates historical pause; shared PID is not execution proof", () => {
  ...
  assert.equal(
    projectState(record, 20000, { isStreaming: true }).effectiveState,
    "running",
  );
  assert.equal(
    projectState(record, 20000, { isStreaming: false }).effectiveState,
    "unknown",
  );
  assert.equal(
    projectState({ state: "running" }, undefined, { isStreaming: false })
      .effectiveState,
    "unknown",
  );
  assert.equal(
    projectState({ ...record, activityState: "needs_attention" }, 9000)
      .effectiveState,
    "needs_attention",
  );
  assert.equal(
    projectState({ state: "completed", lastUpdate: 20000 }, 19000)
      .effectiveState,
    "completed",
  );
  assert.equal(
    projectState({ state: "completed", lastUpdate: 10000 }, 20000)
      .effectiveState,
    "unknown",
  );
});
  ...
  assert.equal(
    (
      await presentRun(
        ...
        "/unused/w",
        [],
        [{ ...child, sessionId: "/foreign" }],
      )
    ).effectiveState,
    "paused",
  );
  const stopped = await presentRun(
    record,
    "/unused/w",
    [],
    [{ ...child, live: false, state: "stopped" }],
  );
  assert.equal(stopped.steps[0].effectiveState, "stopped");
  assert.notEqual(stopped.effectiveState, "running");
  const stopping = await presentRun(
    record,
    "/unused/w",
    [],
    [child, { ...child, runId: "w", index: -1, state: "stopping" }],
  );
...
test("only explicit latest acceptance report flags unmet criteria; tool output and prose cannot", async () => {
  ...
  assert.equal(
    reportsUnmetCriteria("```acceptance-report\ninvalid\n```"),
    false,
  );
  ...
  try {
    ...
    const record = {
      state: "complete",
      lastUpdate: 30000,
      steps: [{ status: "completed", sessionFile }],
    };
    for (const role of ["assistant", "toolResult"]) {
      await writeFile(
        ...
        JSON.stringify({
          type: "message",
          timestamp: 20000,
          message: { role, content: report },
        }) + "\n",
      );
      const view = await presentRun(record, join(root, "w"), []);
      assert.equal(
        view.outcome,
        role === "assistant" ? "reported-unmet" : undefined,
      );
    }
    await writeFile(
      sessionFile,
      [
        ...
        },
        {
          type: "message",
          timestamp: 20000,
          message: { role: "assistant", content: "New result" },
        },
      ]
        .map(JSON.stringify)
        .join("\n"),
    ...
    assert.equal(
      (await presentRun(record, join(root, "w"), [])).outcome,
      undefined,
    );
  } finally {
  ...
  assert.equal(
    (
      await presentRun(
        {
          state: "failed",
          error: "Error: Run 'worker' failed: spawn pi ENOENT",
        },
        "/unused/w",
        [],
      )
...
test("completed children and failed continuation are independent outcomes, without rewriting records", async () => {
  ...
  const partial = await presentRun(
    { ...record, steps: [{ status: "completed" }, { status: "failed" }] },
    "/unused/workflow",
    [],
  );
  assert.deepEqual(partial.childSummary, { total: 2, completed: 1 });
  assert.equal(
    (await presentRun({ ...record, steps: [] }, "/unused/workflow", []))
      .childSummary.completed,
    0,
  );
});

test("timeout classification uses runner diagnostics, not arbitrary output wording", async () => {
  const timeout = await presentRun(
    {
      state: "failed",
      error: "Subagent timed out after 1799248ms.",
      steps: [{ status: "failed" }],
    },
    "/unused/workflow",
    [],
  ...
  assert.equal(
    (
      await presentRun(
        { state: "failed", error: "Validation failed: expected timeout flag" },
        "/unused/workflow",
        [],
      )
    ).workflow.issue,
  ...
  assert.equal(
    (
      await presentRun(
        {
          state: "complete",
          error: "unsupported-continuation: old diagnostic",
        },
        "/unused/workflow",
        [],
      )
...
test("newer matching native child state is used; child completion does not complete workflow", async () => {
  ...
  try {
    ...
    assert.equal(
      (await presentRun(record, join(root, "parent"), [])).effectiveState,
      "unknown",
    );
    const completedParent = await presentRun(
      {
        ...record,
        state: "failed",
        lastUpdate: 30000,
    ...
    assert.equal(
      (
        await presentRun(record, join(root, "parent"), [
          { sessionFile: session, isStreaming: true },
        ])
      ).effectiveState,
      "running",
    );
