...
import {
  prepareRepair,
  applyRepair,
} from "../../scripts/context-mode-repair.mjs";

const installed =
  process.env.MEWPII_CONTEXT_MODE_ROOT ??
  join(homedir(), ".pi/agent/npm/node_modules/context-mode");
let available = true;
try {
  await access(join(installed, "package.json"));
} catch {
  available = false;
}
...
test("context-mode repair: actual MCP cancellation, concurrent isolation and executor pipe settlement", {
  skip: !available,
  timeout: 60_000,
}, async (t) => {
  ...
  try {
    ...
    // The live install may already be patched; never drag its .mewpii-original backups into the
    // isolated copy, or applyRepair would see a stale backup and refuse ("Backup already exists").
    await cp(join(installed, "build"), join(pkg, "build"), {
      recursive: true,
      filter: (src) => !src.endsWith(".mewpii-original"),
    });
    await cp(join(installed, "package.json"), join(pkg, "package.json"));
    await cp(
      join(installed, "server.bundle.mjs"),
      join(pkg, "server.bundle.mjs"),
    );
    // Keep testing the upstream regression after the real deployment is patched.
    for (const file of [
      "build/adapters/pi/mcp-bridge.js",
      "build/executor.js",
      "build/server.js",
      "server.bundle.mjs",
    ]) {
      try {
        await cp(join(installed, `${file}.mewpii-original`), join(pkg, file));
      } catch (error) {
    ...
    const before = await readFile(
      join(installed, "build/adapters/pi/mcp-bridge.js"),
      "utf8",
    );
    ...
    const { bootstrapMCPTools, MCPStdioClient } = await import(
      pathToFileURL(join(pkg, "build/adapters/pi/mcp-bridge.js"))
    );
    const { PolyglotExecutor } = await import(
      pathToFileURL(join(pkg, "build/executor.js"))
    );
    const definitions = new Map();
    bridge = await bootstrapMCPTools(
      { registerTool: (def) => definitions.set(def.name, def) },
      join(pkg, "server.bundle.mjs"),
      {
        ...
        env: {
          ...
          HOME: testHome,
          TMPDIR: root,
          PI_CONFIG_DIR: join(testHome, ".pi"),
          CONTEXT_MODE_PROJECT_DIR: work,
    ...
    await t.test(
      "registered ctx_execute accepts abort; kills only the cancelled process group",
      async () => {
        const controller = new AbortController();
        const pidFile = join(work, "cancel.pid");
        const running = definitions.get("ctx_execute").execute(
          "cancel",
          {
            language: "javascript",
            timeout: 30_000,
        ...
        await until(async () => {
          try {
            return Boolean(await readFile(pidFile, "utf8"));
          } catch {
            return false;
          }
        });
        ...
        ownedPids.add(pid);
        const other = definitions.get("ctx_execute").execute(
          "other",
          {
            language: "javascript",
            timeout: 5000,
    ...
    await t.test(
      "already aborted requests never reach the server",
      async () => {
        const controller = new AbortController();
        controller.abort();
        ...
        await assert.rejects(
          bridge.client.callTool(
            "ctx_execute",
            { language: "javascript", code: "throw Error('must not run')" },
            controller.signal,
          ),
          { name: "AbortError" },
        );
    ...
    await t.test(
      "file and batch executors receive the same per-request cancellation",
      async () => {
        for (const name of ["ctx_execute_file", "ctx_batch_execute"]) {
          ...
          let earlyResult;
          const outcome = definitions
            .get(name)
            .execute(name, args, controller.signal)
            .then(
              (value) => (earlyResult = { value }),
              (error) => (earlyResult = { error }),
            );
          await until(async () => {
            if (earlyResult)
              throw new Error(
                `${name} ended before publishing pid: ${earlyResult.error?.stack ?? JSON.stringify(earlyResult.value)}`,
              );
            try {
              return Boolean(await readFile(pidFile, "utf8"));
            } catch {
    ...
    await t.test(
      "lost RPC times out, sends cancellation and frees pending entries",
      async () => {
        ...
        client.onData(
          Buffer.from(JSON.stringify({ id: frames[0].id, result: {} }) + "\n"),
        );
        assert.equal(
          client.pending.size,
          0,
          "late result resurrected cancelled request",
        );
      },
    );

    await t.test(
      "bridge shutdown rejects pending calls even before the child exits",
      async () => {
        const client = new MCPStdioClient("unused");
    ...
    await t.test(
      "finished command cannot hang forever on inherited output pipes",
      async () => {
        const executor = new PolyglotExecutor({
          projectRoot: work,
          runtimes: { javascript: process.execPath },
        });
    ...
    await t.test(
      "explicit process timeout settles and a subsequent execution still works",
      async () => {
        const executor = new PolyglotExecutor({
          projectRoot: work,
          runtimes: { javascript: process.execPath },
